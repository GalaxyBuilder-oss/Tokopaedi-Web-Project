...
const LoginPage = "./signup-login.html";
  ...
  if (!currPage.includes("/detail.html")) {
    ...
    if (localStorage.getItem("user")) {
      ...
      cartGo.addEventListener("click", () => {
        ...
        alert(
          `You Have ${
            i[parseInt(localStorage.getItem("user_id"))].cart
          } Goods in Cart!`
        );
      });
      ...
      buttonLogin.addEventListener("click", () => {
        document
          .querySelector(".popup-profile-hide")
          .classList.toggle("popup-profile-show");
      });
    } else {
    ...
    window.onscroll = () => {
      if (document.documentElement.scrollTop > 70) {
        ...
        if (document.querySelector(".to-top") == undefined)
    ...
    let tokopaedi = {
      addCategoryToNav: (category, index) => {
        // menambahkan nama kategori ke nav header dan aside(untuk tampilan mobile)
      ...
      addProductToContent: (category, index) => {
        // menampilkan produk dan mengelompokkanya sesuai kategori masing-masing
        let theProduct = {
          image: "",
          title: "",
          description: "",
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (i < product.rating.rate)
      productRate.innerText =
        i < Math.floor(product.rating.rate) ? "star" : "star_half";
...
function user() {
  ...
  if (localStorage.getItem("user") && currPage.includes("/profile.html")) {
    ...
    currPage.includes("/signup-login.html")
  ) {
      ...
      login: () => {
        ...
        formField.addEventListener("submit", () => {
          if (
            ...
            passField.value === userData.password
          ) {
            alert("login");
...
if (
  localStorage.getItem("isCheckDetails?") &&
  currPage.includes("/detail.html")
) {
  produkDetails();
} else if (
  currPage.includes("/profile.html") ||
  currPage.includes("/signup-login.html")
) {
  user();
